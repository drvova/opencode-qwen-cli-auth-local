...
export async function pollForToken(deviceCode, verifier, interval = 2) {
    try {
        ...
        if (!res.ok) {
            ...
            const transientStatus = res.status >= 500 || res.status === 429 || res.status === 408 || res.status === 404;
            if (!errorCode && !json?.access_token && (res.status === 400 || res.status === 404)) {
                return { type: "pending" };
            }
        ...
        if (!text || !text.trim()) {
            return {
                type: "failed",
                error: "empty_oauth_response",
                description: "OAuth token endpoint returned empty body",
                fatal: false,
            };
        }
        const json = extractJsonObjectFromText(text);
        if (!json || typeof json !== "object") {
            ...
            if (lowered.includes("authorization_pending") ||
                lowered.includes("authorization pending") ||
                lowered.includes("pending") ||
                lowered.includes("not yet") ||
                lowered.includes("please wait") ||
                lowered.includes("device code") ||
            ...
            return {
                type: "failed",
                error: "invalid_json_oauth_response",
                description: "OAuth token endpoint returned non-JSON body",
                fatal: false,
            };
        }
        if (LOGGING_ENABLED) {
        ...
        if (!validateTokenResponse(json, "token response")) {
            const looksLikePending = json?.error === "authorization_pending";
            if (looksLikePending) {
                return { type: "pending" };
            }
            return {
        ...
        const transient = lowered.includes("timed out") ||
